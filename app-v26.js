const LS_KEY = 'convoca_app_v6_data'; // cache locale; il dato master può essere Supabase
...
/* =========================================================
   v26 - Supabase Sync unico archivio condiviso
   ========================================================= */
...
let cloudLastLoadedAt = null;

function cloudConfigOk(){
  const cfg = window.CONVOCA_CONFIG || {};
  return !!(cfg.SUPABASE_URL && cfg.SUPABASE_ANON_KEY &&
    !String(cfg.SUPABASE_URL).includes("INCOLLA_QUI") &&
    !String(cfg.SUPABASE_ANON_KEY).includes("INCOLLA_QUI"));
}

function cloudAppId(){
  return (window.CONVOCA_CONFIG && window.CONVOCA_CONFIG.APP_ID) || "convoca-app-principale";
}

function cloudStatus(text, kind="info"){
  let el = document.getElementById("cloudStatus");
  if (!el) {
    el = document.createElement("div");
    el.id = "cloudStatus";
    el.className = "no-print";
    el.style.cssText = "position:fixed;right:10px;bottom:42px;z-index:9999;padding:7px 10px;border-radius:999px;font-size:11px;font-weight:900;box-shadow:0 10px 25px rgba(15,23,42,.20);background:rgba(255,255,255,.92);border:1px solid rgba(216,225,238,.95);color:#142033";
    document.body.appendChild(el);
  }
  el.textContent = text;
  if (kind === "ok") el.style.background = "rgba(220,252,231,.95)";
  if (kind === "warn") el.style.background = "rgba(254,243,199,.95)";
  if (kind === "err") el.style.background = "rgba(254,226,226,.95)";
}

async function cloudInit(){
  try {
    if (!cloudConfigOk()) {
      cloudStatus("Cloud non configurato", "warn");
      return;
    }
    if (!window.supabase || !window.supabase.createClient) {
      cloudStatus("Supabase JS non caricato", "err");
      return;
    }
    const cfg = window.CONVOCA_CONFIG;
    cloudClient = window.supabase.createClient(cfg.SUPABASE_URL, cfg.SUPABASE_ANON_KEY);
    cloudReady = true;
    cloudStatus("Cloud collegato", "ok");
    await cloudLoad();
  } catch(e) {
    console.error(e);
    cloudStatus("Errore cloud", "err");
  }
}
...
async function cloudLoad(){
  ...
  try {
    cloudStatus("Carico dati cloud...", "warn");
    ...
    if (data && data.data) {
      ...
      const localEmpty = !localRaw;
      state = data.data;
      cloudLastLoadedAt = data.updated_at || null;
      localStorage.setItem(LS_KEY, JSON.stringify(state));
      if (typeof renderAll === "function") renderAll();
      if (typeof render === "function") render();
      cloudStatus("Dati cloud aggiornati", "ok");
    } else {
      // Primo avvio: carica sul cloud i dati locali del PC/dispositivo corrente.
      await cloudSaveNow(true);
      cloudStatus("Cloud inizializzato", "ok");
    }
  } catch(e) {
    console.error(e);
    cloudStatus("Errore lettura cloud", "err");
    alert("Errore lettura Supabase: " + (e.message || e));
  }
}
...
function cloudScheduleSave(){
  ...
  cloudSaveTimer = setTimeout(() => cloudSaveNow(false), 650);
}

async function cloudSaveNow(silent=false){
  if (!cloudReady || !cloudClient || cloudSaving) return;
  cloudSaving = true;
  try {
    if (!silent) cloudStatus("Salvataggio cloud...", "warn");
    const payload = {
      id: cloudAppId(),
      data: state,
    ...
    if (error) throw error;
    if (!silent) cloudStatus("Salvato su cloud", "ok");
  } catch(e) {
    console.error(e);
    cloudStatus("Errore salvataggio", "err");
    alert("Errore salvataggio Supabase: " + (e.message || e));
  } finally {
    cloudSaving = false;
  }
}

async function cloudForceUploadLocal(){
  if (!cloudReady) return alert("Supabase non è configurato o non è collegato.");
  if (!confirm("Vuoi caricare sul cloud i dati presenti su questo dispositivo? Fallo dal PC dove vedi i 25 giocatori.")) return;
  await cloudSaveNow(false);
  alert("Dati caricati su Supabase. Ora telefono e PC dovrebbero leggere gli stessi dati.");
}

async function cloudForceDownload(){
  if (!cloudReady) return alert("Supabase non è configurato o non è collegato.");
  await cloudLoad();
  alert("Dati scaricati da Supabase.");
}

function cloudInjectTools(){
  if (document.getElementById("cloudTools")) return;
  const box = document.createElement("div");
  box.id = "cloudTools";
  box.className = "no-print";
  box.style.cssText = "position:fixed;left:8px;bottom:72px;z-index:9999;display:flex;gap:6px;flex-wrap:wrap;max-width:calc(100vw - 16px);";
  box.innerHTML = `
    <button class="btn secondary" style="font-size:10px;padding:6px 8px;border-radius:999px" onclick="cloudForceDownload()">Scarica cloud</button>
    <button class="btn primary" style="font-size:10px;padding:6px 8px;border-radius:999px" onclick="cloudForceUploadLocal()">Carica questo PC sul cloud</button>
  `;
  document.body.appendChild(box);
}

document.addEventListener("DOMContentLoaded", () => {
  setTimeout(cloudInit, 500);
  setTimeout(cloudInjectTools, 900);
});
